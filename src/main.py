...
if __name__ == "__main__":
    ...
    folderConvertor.run_command_on_files(path, ".html")
    path_index = path+ os.sep +"index.html"
    fileUtils.copy_resources(pathUtils.get_runtime_script_path(__file__) + os.sep + "resources", path + os.sep + "resources", [".css", ".png"])
    folderStructureGenerator.generate_html_file(path, path_index)
    ftp.upload_files(path, [".html", ".css", ".png"])
    fileUtils.delete_files(path, [".html", ".css", ".png", ".tmp"])